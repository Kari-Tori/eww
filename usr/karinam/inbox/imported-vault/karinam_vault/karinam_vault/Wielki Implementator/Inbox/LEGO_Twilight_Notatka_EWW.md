---
title: Lego Twilight Notatka Eww
...
[!summary] Mapa etapów
🟩 Etap 0: Oryginał 7680 — baza do modyfikacji · £140–£170🟨 Etap 1: MOC #1 (Bruxxy) — minifig display ~1418 cz. · ~£225–£355 (łącznie)🟪 Etap 2: MOC #2 (IcarusBuilds) — minifig‑scale „UCS” ~11k+ cz. · ~£900–£1700 części
...
🛸 Klasa: G9 Rigger‑class light freighter🏭 Producent: Corellian Engineering Corporation (CEC)🗓️ Okres użycia: Wojny Klonów🎯 Zastosowanie: dyskretne misje, transport ładunku/osób, mody „na szybko” przez Anakina

[!users] Załoga — liczba i stanowiska
👥 Minimalna załoga: 2 (pilot + kopilot)👥 Maksymalna załoga (nominalna): 2 (pilot + kopilot) — dodatkowe siedzisko nawigatora nie zwiększa pola „crew”.👨‍✈️ Pilot: Anakin Skywalker · 🧡 Kopilot/strzelec: Ahsoka Tano · 🤖 Astromech: R2‑D2📦 Pasażerowie/ładunek: według misji; tylna ładownia z wyciągarką
...
⛓️ Wyciągarka w ładowni (cargo winch)🛠️ Liczne modyfikacje polowe wykonywane przez Anakina🛸 (MOC, niekanon) Mini‑hangar pod myśliwiec Anakina jako mod display
...
+1 nawigator (kokpit), gniazdo astromecha
...
Prędkość atmosferyczna

—

~700 km/h

Uwagi: Dane w kolumnie „Legends” pochodzą z materiałów RPG/Legends; w planie budowy oznaczamy je symbolem † gdy są używane operacyjnie.



[!quote] Pozyskanie i utrata statku
🛬 Pozyskanie: przejęty na Teth po wydarzeniach z filmu The Clone Wars (2008); Anakin z Ahsoką wykorzystali go do ucieczki z Rottą, po czym Anakin zlecił naprawy i zatrzymał jednostkę jako "low‑profile" transport.💥 Zniszczenie: utracony na Mandalore w działaniach związanych z Maulem (okres Siege of Mandalore).

[!list] Postacie powiązane (linki — Wookieepedia / LEGO)
...
✧ Słownik nazw i bohaterów

[!info] Nazwy własne — wyróżnienia
Statek: 🛸 G9 Rigger „TWILIGHT” (Corellian Engineering Corp.)Pilot: 👨‍✈️ Anakin Skywalker (CW)Padawanka: 🧡 Ahsoka Tano (Padawan)Astromech: 🤖 R2‑D2Ładunek wyjątkowy: 🟢 Rotta (Hutt juvenile)

Kanon skrótowo

Długość 34.1 m · Hyperdrive kl. 3 · 3 działa laserowe.

Tylna ładownia z wyciągarką. Pochodzenie: Ziro the Hutt. Zniszczenie: Mandalore.

Dokowanie: kanon = ładownia/wyciągarka. Mini‑hangar pod myśliwiec Anakina traktuj jako mod display w MOC.

🧱 Fiszka zestawu – LEGO 7680

[!abstract] Parametry
📅 2008 · 🧪 The Clone Wars · 📴 wycofany ~XI 2009💷 RRP: £68.49 / $99.99📦 Pudełko: ~57.6 × 38.4 × 9.6 cm · 🔢 UPC 673419102728📘 Instrukcje: 2 tomy PDF (LEGO Service)🧩 Liczba części (źródła): 882 (Brickset) · 867 (BrickLink) · ~931 (LEGO BI)
...
[!info] Przyjmujemy inventory z LEGO BI ≈ 931 częściKontrola punktowa: linka x77ac50 · zawiasy/Technic · panele/wedge · tile’e frontowe
...
[!note] Donor: Oryginał 7680 · Dodatkowe sztuki: ~500–600Najczęstsze braki: poszycie kadłuba, spójna paleta Technic, detale kokpitu/ogonaStand: osobny MOC → osobny mini‑BOM
...
[!warning] Brak oficjalnej instrukcji/BOM → reverse‑engineering ze zdjęć/wątkuDominują: elementy nośne i poszycie w spójnej palecie kolorów
...
[!timeline]
T0 — zakup i weryfikacja Oryginału → G0T1 — import MOC #1, zakupy, budowa, stand → G1T2 — plan sub‑assembly MOC #2, batch‑zakupy, build‑log → G2
...
```dataviewjs
const stages = [
  { id: "G0", icon: "🧱", name: "Oryginał", incMin: 140, incMax: 170, totMin: 140, totMax: 170 },
  { id: "G1", icon: "⚙️", name: "MOC #1", incMin: 85,  incMax: 175, totMin: 225, totMax: 355 },
  { id: "G2", icon: "⭐", name: "MOC #2", incMin: 900, incMax: 1700, totMin: 1125, totMax: 2055 }
];
dv.table(["Etap", "Δ £ min", "Δ £ max", "£ total min", "£ total max"],
  stages.map(s => [s.icon + " " + s.name, s.incMin, s.incMax, s.totMin, s.totMax]));
...
TABLE file.link AS "Zamówienie", file.cday AS "Utwo
