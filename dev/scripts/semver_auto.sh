...
dry=0; if_main=0; use_gh="${GH:-0}"
CHANGELOG_PATH="${CHANGELOG_PATH:-CHANGELOG.md}"
VERSION_PATH="${VERSION_PATH:-VERSION}"
for a in "$@"; do case "$a" in --dry-run) dry=1;; --if-main) if_main=1;; --gh) use_gh=1;; *) echo "usage: $0 [--dry-run] [--if-main] [--gh]" >&2; exit 2;; esac; done
...
bump="none"
if grep -Eq '(^|\n)BREAKING CHANGE' <<<"$log" || grep -Eq '(^|\n)(feat|fix|perf|refactor)!' <<<"$log"; then bump="major"
elif grep -Eq '(^|\n)feat(\(|:)' <<<"$log"; then bump="minor"
elif grep -Eq '(^|\n)(fix|perf|refactor)(\(|:)' <<<"$log"; then bump="patch"; fi
[[ "$bump" = "none" ]] && { echo "INFO: no bump since $last_tag"; exit 0; }
...
{ echo "## v${new_ver} — ${today}"; if [[ "$last_tag" == "v0.0.0" ]]; then git log --pretty=format:"- %s (%h)"; else git log "${last_tag}..HEAD" --pretty=format:"- %s (%h)"; fi; echo; } > "$tmp"
