...
| `projects/init/MVP.md` | Kryteria akceptacji dla wersji MVP 0.0.0.1 |
| `AGENTS.md` | Kontekst dla AI coding agents (architektura, workflow) |
| `projects/init/init-eww.sh` | Skrypt inicjalizacyjny środowiska (source w ~/.bashrc) |
...
| `eww-fix-init.sh` | Naprawa uszkodzonego `projects/init/init-eww.sh` |
...
| Plik | Opis |
|------|------|
| `bashrc.eww.snippet` | Fragment do dodania do ~/.bashrc (source projects/init/init-eww.sh) |
...
### dev/tests/ - Testy i notatki Neovim

> Testy i dokumentacja powiązana z konfiguracją Neovim

| Plik | Opis |
|------|------|
| `test_neovim.md` | Dokumentacja scenariuszy testowych dla Neovim |
| `test_neovim.sh` | Skrypt uruchamiający zestaw testów Neovim |
| `test_neovim.yaml` | Konfiguracja YAML dla testów z wykorzystaniem `nvim --headless` |

### projects/init/ - Inicjalizacja

> Katalog zbiera stary indeks repozytorium oraz narzędzia/testy dotyczące `projects/init/init-eww.sh`.

#### projects/init/scripts/

| Skrypt | Opis |
|--------|------|
| `eww-fix-init.sh` | Naprawia kopię `projects/init/init-eww.sh` (przeniesienie, uprawnienia, sanity check) |
| `eww-init-idempotent-example.sh` | Przykład idempotentnego szablonu init na podstawie `lib/idempotent.sh` |

#### projects/init/tests/

| Test | Opis |
|------|------|
| `test_init.bats` | Weryfikuje, że katalog repozytorium istnieje |
| `test_init_behavior.bats` | Sprawdza zachowanie sourcowania `projects/init/init-eww.sh` w powłoce |
...
- **Główne pliki**: `README.md`, `projects/init/init-eww.sh`, `projects/init/MVP.md`, `VERSION`, `CHANGELOG.md`
- **Biblioteki**: `lib/*.sh`
