...
- **Opis:** Wersja dokumentu (wymagana w każdym pliku z frontmatterem)
- **Format:** `MAJOR.MINOR.PATCH` lub `MAJOR.MINOR.PATCH.BUILD`
- **Przykład:** `0.0.0.5`, `1.0.0`
- **Użycie:** Śledzenie zmian w dokumentacji i synchronizacja z tagami/releases
- **Wartości:**
...
### docs/infra/guide/intro.md (przewodnik)
...
### docs/infra/wiki/kubuntu.md (wiki article)
...
eww-frontmatter --validate docs/infra/guide.md

# Dry-run (pokaż bez zapisywania)
eww-frontmatter --dry-run --auto docs/infra/wiki/intro.md
```
