---
tags:
  - #folder_note
  - #automation
  - #development
  - #docs
  - #eww
  - #knowledge
  - #linux
  - #secondbrain
created: 2025-11-21T20:00:00Z
modified: 2025-11-21T23:00:00Z
author: jakubc
title: "📁 #[+#ffb300]style-guide"






owner: jakubc
---

# 📁 #[+#ffb300]style-guide

> **Folder Note** - centralny index dla katalogu `#[+#ffb300]style-guide/`

## 📋 Cel folderu

Katalog `#[+#ffb300]style-guide` zawiera 0 plików i 0 podfolderów.

**Przeznaczenie:**
- Dokumentacja projektowa i techniczna


## 📄 Pliki w folderze

*Brak plików MD w tym folderze.*

## 📂 Podfoldery

*Brak podfolderów.*

## 🔗 Nawigacja

- ⬆️ [[tags/tags|tags]] - Folder nadrzędny
- 🏠 [[infra/infra.md|Infra README]] - Główny index


**Lokalizacja:** `docs/infra/tags/#[+#ffb300]style-guide`  
**Zaktualizowano:** 2025-11-21  
**Typ:** Folder Note

## 📑 Index plików

- [#[+#ffb300]style-guide](#[+#ffb300]style-guide.md)

## 📁 Podfoldery

- [#[+#ffb300]style-guide/](#[+#ffb300]style-guide/#[+#ffb300]style-guide.md)

## 🔗 Backlinks

- [[EWW-MAP]]
- [[GRAPH-MAP]]
- [[GRAPH-CORE]]
- [[GRAPH-BUSINESS]]
- [[GRAPH-CONFIG]]

