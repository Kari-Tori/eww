---
tags:
  - #folder_note
  - #automation
  - #docs
  - #eww
  - #knowledge
  - #linux
  - #scripts
  - #secondbrain
created: 2025-11-21T20:00:00Z
modified: 2025-11-21T21:00:00Z
author: jakubc
title: "📁 min"






owner: jakubc
---

# 📁 min

> **Folder Note** - centralny index dla katalogu `min/`

## 📋 Cel folderu

Katalog `min` zawiera 0 plików i 0 podfolderów.

**Przeznaczenie:**
- Pliki i zasoby ogólnego przeznaczenia


## 📄 Pliki w folderze

*Brak plików MD w tym folderze.*

## 📂 Podfoldery

*Brak podfolderów.*

## 🔗 Nawigacja

- ⬆️ [[lunr/lunr|lunr]] - Folder nadrzędny
- 🏠 [[infra/infra.md|Infra README]] - Główny index


**Lokalizacja:** `site/assets/javascripts/lunr/min`  
**Zaktualizowano:** 2025-11-21  
**Typ:** Folder Note

## 🔗 Backlinks

- [[EWW-MAP]]
- [[GRAPH-MAP]]
- [[GRAPH-CORE]]
- [[GRAPH-BUSINESS]]
- [[GRAPH-CONFIG]]

