...
Centralny przełącznik warstwy dostępowej z PoE, który przejmuje rolę Netgear FS108 i staje się podstawą modernizacji opisanej w [[docs/infra/network/network-deployment|planie wdrożenia]]. Folder zawiera wszystkie informacje operacyjne o urządzeniu w sieci EWW: fizyczną lokalizację, plan kablażu, log wdrożenia oraz zadania.
...
> - **Zależności:** [[docs/infra/network/garynet]], router dual-WAN, zasilanie z UPS 5 kW

## 🧩 Kontekst sieci
- Plan docelowy (dual-WAN + Catalyst) jest opisany w [[docs/infra/network/network-deployment]].
- Bieżące zadania operacyjne i log z dnia 2025-11-23 znajdują się w [[docs/infra/network/network-deployment-2025-11-23]]; najważniejsze wpisy zostały zmirrorowane w pliku [[ops/2025-11-23~initial-setup]].
- Status łańcucha WAN (iPhone → Lenovo Y700 → ASUS DSL-N16 → Netgear) z [[docs/infra/status_sieci]] wskazuje dlaczego Catalyst musi przejąć rolę core switcha.
...
| **Konfiguracja** | VLAN 1 (LAN), VLAN 20 (IoT/PoE), profile portów `Gi1/0/x` | 🟡 draft w [[ports.md]] |
...
- **Szafa serwerowa U42 (pomieszczenie 24 m²)** — Catalyst montujemy wysoko (U38–U42) tuż pod panelem krosowym, frontem do drzwi dla łatwego dostępu do portów i konsoli (info z [[docs/infra/network/network-deployment#🏗️-Lokalizacja-fizyczna-(rack-U42-+-biurko-DEV)|plan wdrożenia]]).
...
## 🗂️ Struktura folderu
- `README.md` — ta karta główna.
- `ports.md` — profilowanie portów i VLAN.
...
- [ ] Wpisać docelowe adresy IP management + konfigurację VLAN do `ports.md`.
- [ ] Dodać schemat patchcordów (diagram/zdjęcia) w `ops/` po montażu.
- [ ] Skonfigurować backup configu do repo (`copy running-config tftp://…`).
- [ ] Uaktualnić [[docs/infra/hardware/hardware.md]] oraz [[docs/infra/network/README]] po zakończeniu migracji.

## 🔗 Powiązane
- [[docs/infra/network/network-deployment]]
- [[docs/infra/network/network-deployment-2025-11-23]]
- [[docs/infra/status_sieci]]
- [[docs/infra/network/garynet]]
- [[docs/infra/hardware/netgear_fs108/netgear_fs108]] (urządzenie zastępowane)
