---
version: 0.0.7
title: "team"
created: 2025-11-21
modified: 2025-11-28
tags:
  - auto-versioned
---

created: 2025-11-21T20:00:00Z
modified: 2025-11-24T22:31:02Z
author: jakubc
---
tags:
  - #folder_note
  - #automation
  - #development
  - #docs
  - #eww
  - #knowledge
  - #linux
  - #secondbrain
title: "📁 team"






owner: jakubc
created: 2025-11-21T20:00:00Z
modified: 2025-11-24T22:31:02Z
author: jakubc
---

# 📁 team

> **Folder Note** - centralny index dla katalogu `team/`

## 📋 Cel folderu

Katalog `team` zawiera 1 plików i 1 podfolderów.

**Przeznaczenie:**
- Dokumentacja projektowa i techniczna


## 📄 Pliki w folderze

- [[ADHD_General]] - ⚡ ADHD – Neuroarchitektura Wielkiej Mocy

## 📂 Podfoldery

- 📂 [[karinam/karinam|karinam]] - Subfolder

## 🔗 Nawigacja

- ⬆️ [[infra/infra|docs/infra]] - Folder nadrzędny
- 🏠 [[docs/INDEX|INDEX]] - Główny index


**Lokalizacja:** `docs/team`  
**Zaktualizowano:** 2025-11-21  
**Typ:** Folder Note

## 📑 Index plików

- [ADHD_General](ADHD_General.md)

## 📁 Podfoldery

- [karinam/](karinam/karinam.md)

## 🔗 Backlinks

- [[EWW-MAP]]
- [[GRAPH-MAP]]
- [[GRAPH-CORE]]
- [[GRAPH-BUSINESS]]
- [[GRAPH-CONFIG]]
