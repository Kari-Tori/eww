...
grep -q "Wersja:" "$README" || { echo "[FAIL] README missing 'Wersja:' line"; fail_count=$((fail_count+1)); }
...
grep -q "## Quick start" "$README" || { echo "[FAIL] README missing '## Quick start' section"; fail_count=$((fail_count+1)); }
grep -q "## Funkcje i lista weryfikacji" "$README" || { echo "[FAIL] README missing '## Funkcje i lista weryfikacji'"; fail_count=$((fail_count+1)); }
