  ...
  if [[ $show -eq 1 ]]; then
    if [[ -r "$LIB_BANER" ]]; then
      ...
      printf "╰─ %s • %s • repo:%s • cfg:%s [MIN]\n\n" \
        ...
        "$EWW_ROOT" "$EWW_ROOT/projects/init/init-eww.sh"
    fi
  fi
}
eww::init
INIT
bash -n "$TMP"
mv "$TMP" "$ROOT/projects/init/init-eww.sh"
chmod 644 "$ROOT/projects/init/init-eww.sh"
...
eww::baner() {
  ...
  cfg="${repo}/projects/init/init-eww.sh"
