...
ensure_lock(){
  ...
  flock -n "${__eww_lock_fd}" || { log "ALREADY RUNNING $name"; exit 0; }
...
ensure_file(){
  ...
  local want cur; want="$(_sha "$tmp")"; cur="$(_sha "$path")"
  if [[ "$want" != "$cur" ]]; then
    install -D -m "$mode" -o "$owner" -g "$group" "$tmp" "$path"
    chg "file $path updated"
  else
    rm -f "$tmp"; ok "file $path already desired"
  fi
}
...
ensure_symlink(){
  ...
  local cur=""; [[ -L "$link" ]] && cur="$(readlink -f "$link" || true)"
...
ensure_pkg(){
  command -v apt-get >/dev/null 2>&1 || { skp "ensure_pkg: non-Debian"; return 0; }
  local updated="${EWW_APT_UPDATED:-0}"
  for p in "$@"; do
    if dpkg -s "$p" >/dev/null 2>&1; then
      ok "pkg $p present"
    else
      if [[ "$updated" -eq 0 ]]; then run sudo apt-get update -y; updated=1; export EWW_APT_UPDATED=1; fi
