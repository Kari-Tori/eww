#!/usr/bin/env bash
set -Eeuo pipefail
usage(){ echo "usage: $0 [major|minor|patch|X.Y.Z]"; exit 2; }
bump(){ IFS=. read -r MA MI PA <<<"$1"; case "$2" in major) ((MA++)); MI=0; PA=0;; minor) ((MI++)); PA=0;; patch) ((PA++));; *) exit 2;; esac; echo "${MA}.${MI}.${PA}"; }
log_between(){ from="$1"; if [[ "$from" == "v0.0.0" || -z "$from" ]]; then git log --pretty=format:"- %s (%h)"; else git log "${from}..HEAD" --pretty=format:"- %s (%h)"; fi; }
mode="${1:-}"; [[ -z "$mode" ]] && usage
...
if [[ "$mode" =~ ^(major|minor|patch)$ ]]; then new="$(bump "$current" "$mode")"
elif [[ "$mode" =~ ^[0-9]+\.[0-9]+\.[0-9]+$ ]]; then new="$mode"
else usage; fi
echo "$new" > VERSION
today="$(date +%F)"
notes="$(mktemp)"; { echo "## v${new} — ${today}"; log_between "$last_tag"; echo; } > "$notes"
